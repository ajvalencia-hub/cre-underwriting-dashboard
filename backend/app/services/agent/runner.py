...
from app.models import AgentMessage, AgentProposal, AgentThread, AgentToolCall, Deal
...
from app.services.agent.tools.write_tools import ProposalValidationError
...
# Tools whose `values` argument defaults to the CURRENT deal's inputs when
# the model omits it — the same scoping rule as above, applied to the
# input map instead of the deal id.
_DEAL_VALUES_DEFAULT_TOOLS = {"solve"}
        ...
        for call in result.tool_calls:
            ...
            if tool_def is None:
                ...
                if call.name in _COMPUTE_FAMILY and compute_call_count >= MAX_COMPUTE_CALLS_PER_TURN:
                    ...
                    if call.name in _DEAL_VALUES_DEFAULT_TOOLS and not isinstance(
                        call_arguments.get("values"), dict
                    ):
                        deal = db.get(Deal, thread.deal_id)
                        call_arguments = {**call_arguments, "values": dict(deal.inputs or {}) if deal else {}}
                    try:
                ...
                try:
                    ...
                    payload = {"error": str(exc), "invalid": exc.errors}
