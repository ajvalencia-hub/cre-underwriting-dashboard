...
def _validation_error(values: dict) -> dict | None:
    """The engine's input validation (services/proforma/input_validation.py)
    as a tool error result, or None when `values` pass. Used by the tools
    whose engine calls would otherwise swallow the error per point (solve's
    scan, the sensitivity grid) and report a misleading "no solution"."""
    _, _, input_errors = input_validation.validate_inputs(values)
    if not input_errors:
        return None
...
def solve(
    ...
    invalid = _validation_error(values)
...
def run_sensitivity(db: Session, baseValues: dict, drivers: list[dict], outputFieldIds: list[str]) -> dict:
    # The grid turns a non-computable point into a per-point warning; invalid
    # BASE values would make every point fail the same way, so they are one
    # tool error up front (same message shape as compute's).
    invalid = _validation_error(baseValues)
    if invalid is not None:
        return invalid
    total_points = 1
    for d in drivers:
