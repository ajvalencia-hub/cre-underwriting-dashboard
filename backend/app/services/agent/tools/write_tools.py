...
# The engine's own numeric family (currency/percent/number/multiple/years).
_NUMERIC_TYPES = input_validation.NUMERIC_TYPES


class ProposalValidationError(ValueError):
    """The proposed changes fail the engine's input validation
    (services/proforma/input_validation.py) — e.g. a non-numeric cell in a
    table field. The runner turns this into a tool ERROR result (no proposal
    row), so the model sees the named fields and can retry."""

    def __init__(self, errors: list[str]):
        self.errors = errors
        super().__init__(f"Invalid input values: {'; '.join(errors)}")
...
def _clean_changes(changes: dict) -> tuple[dict, list[str]]:
    if not isinstance(changes, dict):
        raise ProposalValidationError(["changes must be an object of field id -> value"])
    ...
    for field_id, value in changes.items():
        ...
        if field_type in _NUMERIC_TYPES and isinstance(value, bool):
    ...
    # Whatever survived the per-field screen must also pass the engine's own
    # validator (table cells, numeric text) — the same check compute runs.
    _, _, errors = input_validation.validate_inputs(clean)
    if errors:
...
def _preview(current_values: dict, changes: dict) -> tuple[dict | None, list[str]]:
    trial = {**(current_values if isinstance(current_values, dict) else {}), **changes}
    try:
