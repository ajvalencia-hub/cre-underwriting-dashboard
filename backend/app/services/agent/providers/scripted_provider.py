...
def chat(messages: list[Message], tools: list[ToolSpec], system: str, model: str) -> ChatResult:
    ...
    if last_tool and last_tool[0] == "get_deal":
        ...
        if any(kw in user_text for kw in _TARGET_KEYWORDS):
            return ChatResult(
                text="",
                # `values` is deliberately omitted — the runner defaults it to
                # the thread's deal inputs, the path a real model takes most.
                tool_calls=[ToolCall(
                    id="call_solve", name="solve",
                    arguments={
                        "targetInput": "exitCapRatePct", "outputMetric": "leveredIrr",
                        "targetValue": 0.15, "bounds": [0.03, 0.12],
                    },
                )],
                usage=Usage(10, 10), stop_reason="tool_use",
            )
    ...
    if last_tool and last_tool[0] == "solve":
        field_value = last_tool[1].get("solvedValue")
