...
def search(q: str = "", db: Session = Depends(get_db)):
    """Archived deals are excluded from every deal-scoped group (deals,
    ...
    note_deals = {
        ...
        for d in db.execute(
            select(Deal).where(
                Deal.id.in_({n.deal_id for n in notes}), Deal.archived_at.is_(None)
            )
        ).scalars()
    } if notes else {}
    # A note whose deal is archived (absent from note_deals) drops out too.
    notes = [
        n for n in notes
    ...
    return {"query": q, "groups": groups, "typeFilter": type_filter, "tagFilter": tag_filter}
