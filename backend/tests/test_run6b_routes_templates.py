...
def test_sheet_grid_happy_path_and_unknown_sheet(client):
    ...
    b2 = body["rows"][1][1]
    # The target's grid cells also carry numberFormat.
    assert {k: b2[k] for k in ("ref", "value", "isFormula")} == {
        "ref": "B2", "value": "=B1*2", "isFormula": True
    }
...
def test_sheet_grid_clamps_max_rows_and_cols(client, monkeypatch):
    ...
    def spy(path, sheet_name, max_rows, max_cols, start_row=1):
...
def test_sheet_grid_missing_stored_file_is_404(client, session_factory):
    # Port: the target's missing-template-file convention is 404 with an
    # "upload it again" detail (mappings preview, recalc-check), not Run 6's 410.
    created = _upload_template(client)
