...
# --- table-end guard (port note) ---------------------------------------------
# The table-end break above is deliberately a TWO-row lookahead
# (rent_roll_parser._TABLE_END_LOOKAHEAD): a one-row version truncated any
# roll whose mid-table subtotal is followed by a blank spacer row before the
# next unit-type block. These lock that in.


def _roll_with(rows, tmp_path, name="guard.xlsx"):
    import openpyxl

    wb = openpyxl.Workbook()
    ws = wb.active
    ws.append(["Unit", "Unit Type", "SF", "Tenant", "Rent"])
    for row in rows:
        ws.append(row)
    path = tmp_path / name
    wb.save(path)
...
def test_mid_table_subtotal_followed_by_more_units_is_not_truncated(tmp_path):
    rows = _roll_with(
        [
            ["101", "1BR/1BA", 650, "Ada", 1400],
            ["102", "1BR/1BA", 650, "Bea", 1420],
            ["Subtotal 1BR/1BA", None, 1300, None, 2820],
            ["201", "2BR/2BA", 950, "Cy", 1900],
            ["202", "2BR/2BA", 950, "Di", 1950],
            ["Total", None, 3200, None, 6670],
        ],
        tmp_path,
    )
    assert [r["unit"] for r in rows] == ["101", "102", "201", "202"]


def test_subtotal_then_blank_spacer_then_more_units_is_not_truncated():
    """The case a one-row lookahead gets wrong: subtotal, blank spacer,
    next unit-type block. Fed to parse_rows directly — excel_extractor
    drops fully blank rows, but PDF-sourced grids (pdf_extractor) and
    partially blank spacer rows do reach the parser."""
    headers = ["Unit", "Unit Type", "SF", "Tenant", "Rent"]
    data_rows = [
        ["101", "1BR/1BA", 650, "Ada", 1400],
        ["Subtotal 1BR/1BA", None, 650, None, 1400],
        [None, None, None, None, None],
        ["201", "2BR/2BA", 950, "Cy", 1900],
        ["Average", None, 800, None, 1650],
        [None, None, None, None, None],
        [None, None, None, None, None],
        ["Notes", None, None, None, None],
    ]
    rows = rent_roll_parser.parse_rows(headers, data_rows, "spacer.pdf", "p1")["rows"]
    assert [r["unit"] for r in rows] == ["101", "201"]


def test_grand_total_followed_by_non_units_ends_the_table(tmp_path):
    """And the break still fires: a grand total followed by two rows that
    are not units stops parsing, even when later rows would otherwise
    parse as (phantom) units."""
    rows = _roll_with(
        [
            ["101", "1BR/1BA", 650, "Ada", 1400],
            ["Total", None, 650, None, 1400],
            ["Expenses", None, None, None, None],
            ["Taxes", None, None, None, 8000],
            ["Phantom", "1BR/1BA", 650, "Nope", 1],
        ],
        tmp_path,
    )
    assert [r["unit"] for r in rows] == ["101"]


def test_roll_without_sf_column_survives_a_mid_table_subtotal(tmp_path):
    """_row_looks_like_unit falls back to the rent column when the roll has
    no SF column at all — otherwise every row fails the 'real figure' test
    and the first subtotal would end the table."""
    import openpyxl

    wb = openpyxl.Workbook()
    ws = wb.active
    ws.append(["Unit", "Unit Type", "Tenant", "Rent"])
    for row in [
        ["101", "1BR/1BA", "Ada", 1400],
        ["Subtotal 1BR/1BA", None, None, 1400],
        ["201", "2BR/2BA", "Cy", 1900],
        ["Total", None, None, 3300],
    ]:
        ws.append(row)
    path = tmp_path / "nosf.xlsx"
    wb.save(path)
    grid = excel_extractor.extract_grid(path, "xlsx")
    parsed = rent_roll_parser.parse_rows(grid["headers"], grid["rows"], "nosf.xlsx", grid["sheet"])
    assert [r["unit"] for r in parsed["rows"]] == ["101", "201"]
