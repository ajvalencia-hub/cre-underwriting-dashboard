...
def test_solve_tool_happy_path(db, analytic):
    ...
    out = read_tools.solve(
        db, targetInput="exitCapRatePct", outputMetric="leveredIrr",
        targetValue=target, values=analytic, bounds=[0.05, 0.12],
    )
    assert out["solvedValue"] == pytest.approx(0.08, abs=1e-3)
    assert out["achievedMetric"] == pytest.approx(target, abs=1e-4)
    assert out["targetInput"] == "exitCapRatePct"
    assert out["outputMetric"] == "leveredIrr"


def test_solve_tool_default_bounds_come_from_schema_or_current_value(db, analytic):
    from app.services.proforma import engine as engine_mod

    target = engine_mod.compute(analytic)["outputs"]["leveredIrr"]
    out = read_tools.solve(
        db, targetInput="exitCapRatePct", outputMetric="leveredIrr", targetValue=target, values=analytic,
    )
    assert out["solvedValue"] == pytest.approx(0.08, abs=1e-3)
    lo, hi = out["scannedRange"]
    assert lo < 0.08 < hi


def test_solve_tool_no_crossing_is_a_typed_result_not_an_error(db, analytic):
    out = read_tools.solve(
        db, targetInput="exitCapRatePct", outputMetric="leveredIrr",
        targetValue=5.0, values=analytic, bounds=[0.05, 0.12],
    )
    assert "error" not in out
    assert out["solvedValue"] is None
    assert out["reason"] == "no_crossing"


def test_solve_tool_bad_requests_are_error_dicts(db, analytic):
    unknown_input = read_tools.solve(
        db, targetInput="notAField", outputMetric="leveredIrr", targetValue=0.1, values=analytic,
    )
    assert "error" in unknown_input

    unknown_metric = read_tools.solve(
        db, targetInput="exitCapRatePct", outputMetric="notAMetric", targetValue=0.1, values=analytic,
    )
    assert "error" in unknown_metric

    bad_bounds = read_tools.solve(
        db, targetInput="exitCapRatePct", outputMetric="leveredIrr",
        targetValue=0.1, values=analytic, bounds=[0.12, 0.05],
    )
    assert "error" in bad_bounds

    no_values = read_tools.solve(db, targetInput="exitCapRatePct", outputMetric="leveredIrr", targetValue=0.1)
    assert "error" in no_values


def test_solve_tool_spec_makes_values_optional():
    spec = ALL_TOOLS["solve"].parameters
    assert set(spec["required"]) == {"targetInput", "outputMetric", "targetValue"}
    assert "values" in spec["properties"]
    assert "bounds" in spec["properties"]
