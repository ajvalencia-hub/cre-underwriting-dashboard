...
# ---------------------------------------------------------------------------
# solve: `values` defaults to the thread's deal inputs (goal-seek backed)
# ---------------------------------------------------------------------------

def test_solve_without_values_uses_the_threads_deal_inputs(db, thread, deal, monkeypatch):
    _install(monkeypatch, [
        ChatResult(
            text="",
            tool_calls=[ToolCall(
                id="c1", name="solve",
                arguments={"targetInput": "exitCapRatePct", "outputMetric": "leveredIrr", "targetValue": 0.15},
            )],
            usage=Usage(1, 1), stop_reason="tool_use",
        ),
    ...
    out = runner.run_turn(db, thread, "what exit cap hits a 15% irr?")

    result = out["toolCalls"][0]["result"]
    assert "error" not in result
    assert result["targetInput"] == "exitCapRatePct"
    # A lower exit cap raises the IRR, so a crossing exists below the 8% on file.
    assert result["solvedValue"] is not None
    assert result["solvedValue"] < deal.inputs["exitCapRatePct"]
    # The log records what the MODEL sent, not the injected inputs.
    assert "values" not in out["toolCalls"][0]["arguments"]
