...
def test_market_search_stays_literal_in_both_directions(client):
    """LIKE metacharacters must stay literal after the bidirectional
    rewrite: a stored comp market containing `%`/`_` may widen the SQL
    prefilter's reverse arm, but the Python predicate still has the final
    say, so it never leaks into unrelated searches."""
    client.post("/api/comps/sale", json={"name": "Wild", "market": "%", "price": 1})
    client.post("/api/comps/sale", json={"name": "Under", "market": "M_ami", "price": 1})
    client.post("/api/comps/sale", json={"name": "Plain", "market": "Miami", "price": 1})

    assert [c["name"] for c in client.get("/api/comps/sale", params={"market": "Miami"}).json()] == ["Plain"]
    assert [c["name"] for c in client.get("/api/comps/sale", params={"market": "M_ami"}).json()] == ["Under"]
    assert [c["name"] for c in client.get("/api/comps/sale", params={"market": "%"}).json()] == ["Wild"]
