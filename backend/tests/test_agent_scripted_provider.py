...
def test_target_intent_calls_solve_after_get_deal():
    ...
    assert result.tool_calls[0].arguments["targetInput"] == "exitCapRatePct"
    assert result.tool_calls[0].arguments["outputMetric"] == "leveredIrr"
    assert result.tool_calls[0].arguments["targetValue"] == 0.15
    # `values` is left to the runner, which defaults it to the deal's inputs.
    assert "values" not in result.tool_calls[0].arguments
...
def test_after_solve_proposes_the_change():
    messages = [
        ...
        _tool_message("c2", {"solvedValue": 0.079, "achievedMetric": 0.15}),
    ]
    result = scripted_provider.chat(messages, [], "system", "scripted-v1")
    assert result.stop_reason == "tool_use"
...
def test_after_propose_ends_turn_citing_the_proposal():
    messages = [
        ...
        _tool_message("c1", {"inputs": {}}),
        Message(role="assistant", tool_calls=[ToolCall(id="c2", name="solve", arguments={})]),
        _tool_message("c2", {"solvedValue": 0.079}),
