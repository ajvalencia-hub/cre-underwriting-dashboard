...
describe('isOutputVisibleFor', () => {
  ...
  it('hides the build-to-sell outputs on acquisitions (for-sale is development-only)', () => {
    expect(isOutputVisibleFor('grossMarginPct', 'acquisition')).toBe(false)
    expect(isOutputVisibleFor('selloutYears', 'acquisition')).toBe(false)
    expect(isOutputVisibleFor('grossMarginPct', 'development')).toBe(true)
  })
