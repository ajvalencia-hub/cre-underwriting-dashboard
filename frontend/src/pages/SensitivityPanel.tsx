...
export default function SensitivityPanel({ schema, template, mappingProfileId, baseValues }: SensitivityPanelProps) {
  ...
  const eligibleDrivers = driverCandidates.filter((f) => mappedFieldIds.has(f.id))
  const eligibleOutputs = schema.outputs.filter((m) => mappedFieldIds.has(m.id))
  ...
  if (!template || !mappingProfileId) {
    return (
      <div className="max-w-4xl text-sm text-slate-500">
        Upload a template and save a mapping profile under "2. Template &amp; Mapping" to run
  ...
  return (
    <div className="max-w-4xl">
      ...
      <p className="mt-1 text-slate-500">
        Sweep 1–2 mapped inputs across a range and see how mapped output metrics respond. Each grid
        point is a real server-side recalculation of your template — not a separate estimate.
      </p>
